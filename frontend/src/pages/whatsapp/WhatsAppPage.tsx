...
const WhatsAppPage: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="min-w-0">
                    <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-primary flex items-center gap-2">
                        <MessageSquare className="w-6 h-6 sm:w-8 sm:h-8 text-emerald-600 dark:text-emerald-400 shrink-0" />
                        <span>Chatbot de WhatsApp por Sucursal</span>
                    </h1>
                ...
                <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto justify-end">
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-2 bg-card border rounded-lg text-xs sm:text-sm font-medium hover:bg-accent transition-colors shadow-sm cursor-pointer"
                    ...
                    {isConnected ? (
                        ...
                        >
                            <Power className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Desconectar
                        </button>
                    ) : (
                        <button
                            onClick={() => connectMutation.mutate()}
                            disabled={connectMutation.isPending || isConnecting}
                            className="flex items-center gap-2 px-3.5 sm:px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs sm:text-sm font-semibold transition-colors shadow-sm cursor-pointer disabled:opacity-50"
                        >
                            <QrCode className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> {isConnecting ? 'Conectando...' : 'Vincular WhatsApp'}
                        </button>
                    )}
                </div>
            </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs text-muted-foreground">
                    ...
                    {isRestrictedBranchUser && (
                        <span className="flex items-center gap-1 text-primary font-medium">
                            <Lock className="w-3.5 h-3.5 shrink-0" /> Acceso restringido a tu sucursal
                        </span>
                    )}
                </div>
                    ...
                    {availableBranches.map(s => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between text-xs text-muted-foreground pt-1.5 border-t border-border/50 min-w-0 gap-2">
                                    <span className="truncate">{s.ciudad?.nombre || 'Central'}</span>
            ...
            <div className="overflow-x-auto pb-1 border-b no-scrollbar scrollbar-thin">
                <div className="flex items-center gap-1 sm:gap-2 min-w-max">
                    ...
                    >
                        <Smartphone className="w-4 h-4 shrink-0" /> Estado & Conexión
                    </button>
                    <button
                    ...
                    >
                        <Bot className="w-4 h-4 shrink-0" /> Menús y Comandos
                    </button>
                    <button
                    ...
                    >
                        <Activity className="w-4 h-4 shrink-0" /> Registro de Actividad ({logsData?.length || 0})
                    </button>
                </div>
            </div>
                    ...
                    <div className="lg:col-span-2 space-y-4 sm:space-y-6">
                        <div className="bg-card border rounded-xl p-4 sm:p-6 shadow-sm space-y-4">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                ...
                                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold w-fit shrink-0 ${
                                    isConnected
                            ...
                            {isConnected ? (
                                <div className="p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl space-y-3">
                                    <div className="flex items-center gap-3">
                                        <div className="p-2.5 sm:p-3 bg-emerald-600 text-white rounded-xl shrink-0">
                                ...
                                <div className="p-4 sm:p-6 bg-amber-500/5 border border-amber-500/30 rounded-xl flex flex-col md:flex-row items-center gap-4 sm:gap-6">
                                    <div className="p-2 bg-white rounded-xl shadow-md shrink-0 border">
                                        <img src={statusData.qrCode} alt="Código QR WhatsApp" className="w-48 h-48 sm:w-56 sm:h-56 max-w-full object-contain" />
                                    </div>
                                ...
                                <div className="p-6 sm:p-8 border rounded-xl text-center space-y-3">
                                ...
                                <div className="p-6 sm:p-8 border rounded-xl text-center space-y-4">
                                    <XCircle className="w-10 h-10 text-muted-foreground mx-auto opacity-50" />
                                    <div>
                                        <div className="text-sm sm:text-base font-bold text-foreground">El bot de {currentSucursal?.nombre} está desconectado</div>
                                    ...
                                    <button
                                        ...
                                        className="w-full sm:w-auto px-6 py-2.5 bg-primary text-primary-foreground font-bold rounded-lg text-sm hover:opacity-90 transition-all cursor-pointer shadow-md inline-flex items-center justify-center gap-2"
                                    >
                                        <QrCode className="w-4 h-4 shrink-0" /> Generar QR de {currentSucursal?.nombre}
                                    </button>
                                </div>
                            )}
                        </div>
                                ...
                                <div className="md:col-span-7 space-y-1.5">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2">
                                        ...
                                        <button
                                            ...
                                            className="px-4 py-2.5 bg-primary text-primary-foreground text-sm font-bold rounded-lg hover:opacity-90 transition-colors disabled:opacity-50 cursor-pointer shrink-0 flex items-center justify-center gap-1.5 shadow-sm"
                                        >
            ...
            {activeTab === 'configuracion' && (
                <div className="bg-card border rounded-xl p-4 sm:p-6 shadow-sm space-y-6 max-w-4xl">
                    <div>
                        <h2 className="text-sm sm:text-base font-bold text-foreground flex items-center gap-2">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between p-3 sm:p-3.5 border rounded-xl bg-muted/20 gap-3">
                            <div>
                                <div className="text-xs sm:text-sm font-bold text-foreground">Respuestas Automáticas Activas</div>
                                <div className="text-[11px] sm:text-xs text-muted-foreground">Permite que el bot responda mensajes entrantes automáticamente.</div>
                            </div>
                            <input
                                type="checkbox"
                        ...
                        <div className="flex items-center justify-between p-3 sm:p-3.5 border rounded-xl bg-muted/20 gap-3">
                            <div>
                                <div className="text-xs sm:text-sm font-bold text-foreground">Ignorar Mensajes de Grupos y Canales (Recomendado)</div>
                                <div className="text-[11px] sm:text-xs text-muted-foreground">El bot nunca responderá en grupos o canales para evitar spam o bloqueos.</div>
                            </div>
                            <input
                                type="checkbox"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="border rounded-xl p-4 bg-card space-y-3">
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        {configState.customCatalogPdf && (
                                            <span className="px-2 py-0.5 bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-400 border border-purple-300 text-[10px] font-bold rounded-full shrink-0">
                                                PDF Activo
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    {configState.customCatalogPdf ? (
                                        <div className="flex items-center justify-between p-2.5 bg-muted/30 border rounded-lg gap-2">
                                            ...
                                            <button
                                                ...
                                                className="text-xs text-red-600 hover:text-red-700 font-semibold px-2 py-1 hover:bg-red-50 dark:hover:bg-red-950/40 rounded transition-colors cursor-pointer shrink-0"
                                            >
                                                Usar Dinámico
                                            </button>
                                        </div>
                        ...
                        <div className="space-y-4 pt-2">
                            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 pb-2 border-b">
                                ...
                                <button
                                    ...
                                    className="px-3.5 py-1.5 bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 font-bold rounded-lg text-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer shrink-0 w-full sm:w-fit"
                                >
                                ...
                                {(configState.bankAccounts && configState.bankAccounts.length > 0) ? (
                                    configState.bankAccounts.map((account, index) => (
                                        <div
                                            ...
                                            className={`border rounded-xl p-3.5 sm:p-4 transition-all shadow-sm ${
                                        ...
                                        >
                                            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 pb-3 mb-3 border-b">
                                                <div className="flex items-center gap-2 min-w-0 flex-wrap">
                                                    <div className="p-1.5 bg-primary/10 text-primary rounded-lg text-xs font-bold font-mono shrink-0">
                                                        #{index + 1}
                                                    </div>
                                                    ...
                                                    {account.qrImage && (
                                                        <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-800 text-[10px] font-bold rounded-full flex items-center gap-1 shrink-0">
                                                ...
                                                <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-t-0 border-border/40">
                                                    ...
                                                    <button
                                                        ...
                                                        className="text-red-500 hover:text-red-700 p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-950/50 transition-colors cursor-pointer"
                                                        ...
                                                        {account.qrImage ? (
                                                            <div className="flex items-center gap-2 w-full">
                                                                <button
                                                                    ...
                                                                    className="px-2 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded text-xs font-semibold flex items-center justify-center gap-1 cursor-pointer truncate flex-1"
                                                                >
                                                            ...
                                                            <label className="w-full p-1.5 border border-dashed border-primary/40 hover:border-primary rounded-lg text-center cursor-pointer transition-colors bg-primary/5 hover:bg-primary/10 flex items-center justify-center gap-1 text-xs text-primary font-medium">
                                                                <Upload className="w-3.5 h-3.5 shrink-0" /> Subir QR
                                                                <input
                                                                    type="file"
                                                                    accept="image/*"
                                                                    className="hidden"
                        ...
                        <div className="pt-4 flex justify-end">
                            <button
                                ...
                                className="w-full sm:w-auto px-6 py-2.5 bg-primary text-primary-foreground font-bold rounded-lg text-sm hover:opacity-90 transition-all cursor-pointer shadow-md disabled:opacity-50 flex items-center justify-center gap-2"
                            >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="border rounded-xl p-4 bg-muted/10 space-y-2">
                                <h3 className="font-bold text-xs text-primary flex items-center gap-1.5">
                                    <Users className="w-4 h-4 shrink-0" /> Menú para Clientes Nuevos / Público
                                </h3>
                                <pre className="text-[11px] sm:text-xs p-3 bg-background border rounded-lg overflow-x-auto whitespace-pre-wrap break-words font-mono text-muted-foreground leading-relaxed">
...
💬 O escribe directamente el nombre de un producto para ver su precio y stock disponible.`}
                            ...
                            <div className="border rounded-xl p-4 bg-muted/10 space-y-2">
                                <h3 className="font-bold text-xs text-primary flex items-center gap-1.5">
                                    <ShieldCheck className="w-4 h-4 shrink-0" /> Menú para Clientes y Personal Registrado
                                </h3>
                                <pre className="text-[11px] sm:text-xs p-3 bg-background border rounded-lg overflow-x-auto whitespace-pre-wrap break-words font-mono text-muted-foreground leading-relaxed">
...
6️⃣ 📊 Resumen Ejecutivo del Día (Administración)`}
            ...
            {activeTab === 'mensajes' && (
                <div className="bg-card border rounded-xl shadow-sm overflow-hidden space-y-4 p-4 sm:p-6">
                    <div className="flex items-center justify-between gap-2">
                        <div>
                            <h2 className="text-sm sm:text-base font-bold text-foreground flex items-center gap-2">
                        ...
                        <button
                            onClick={() => refetchLogs()}
                            className="p-2 border rounded-lg hover:bg-accent text-muted-foreground cursor-pointer shrink-0"
                    ...
                    <div className="border rounded-xl overflow-hidden divide-y">
                        {logsData && logsData.length > 0 ? (
                            logsData.map((log) => (
                                <div key={log.id} className="p-3 sm:p-3.5 text-xs flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-3 hover:bg-muted/20 transition-colors">
                                    ...
                                    <span className="text-[10px] text-muted-foreground shrink-0 font-mono self-end sm:self-auto">
            ...
            {selectedQrPreview && (
                ...
                >
                    <div
                        className="bg-card border rounded-2xl p-5 sm:p-6 max-w-sm w-full space-y-4 shadow-2xl text-center"
                    ...
                    >
                        ...
                        <div className="p-3 bg-white rounded-xl border shadow-inner">
                            <img src={selectedQrPreview} alt="QR Preview" className="w-full max-h-72 object-contain rounded-lg" />
                        </div>
                        <button
                            onClick={() => setSelectedQrPreview(null)}
                            className="w-full py-2.5 bg-primary text-primary-foreground font-bold rounded-lg text-sm hover:opacity-90 transition-opacity cursor-pointer"
                        >
                            Cerrar
                        </button>
                    </div>

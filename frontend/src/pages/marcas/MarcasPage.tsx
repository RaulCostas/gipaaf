...
import type { Marca } from '../../api/marcaService';
import { 
...
const MarcasPage: React.FC = () => {
    ...
    const { data: marcasList, isLoading } = useQuery({
        queryKey: ['marcasList'],
    ...
    const handlePrint = () => {
        ...
        printData('Reporte de Marcas', exportColumns, filteredMarcas);
    };

    const handleExportPDF = () => {
        if (!marcasList) return;
        exportToPDF('Reporte de Marcas', exportColumns, filteredMarcas, 'marcas_reporte');
    };

    const handleExportExcel = () => {
        if (!marcasList) return;
        exportToExcel(excelColumns, filteredMarcas, 'marcas_reporte');
    };

    const createMutation = useMutation({
        mutationFn: marcaService.create,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['marcasList'] });
            setIsEditing(false);
            setCurrentMarca({});
    ...
    const updateMutation = useMutation({
        mutationFn: (data: Marca) => marcaService.update(data.id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['marcasList'] });
            setIsEditing(false);
            setCurrentMarca({});
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const { submarcas, padre, ...safeData } = currentMarca as any;

        if (currentMarca.id) {
            updateMutation.mutate(safeData as Marca, {
                onSuccess: () => toast.success('Marca actualizada con éxito')
            });
        } else {
    ...
    if (isLoading) return <div className="p-6 text-center text-muted-foreground animate-pulse">Cargando marcas...</div>;
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex items-center flex-wrap gap-3">
                    ...
                    <button
                        onClick={() => { setIsEditing(true); setCurrentMarca({}); }}
            ...
            <Modal
                ...
                title={
                    <span className="flex items-center gap-2 text-primary">
                        ...
                        {currentMarca.id ? 'Editar Marca' : 'Nueva Marca'}
                    </span>
                }
            >
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="space-y-5">
                        ...
                        <div className="flex items-center justify-between p-4 border rounded-lg bg-background hover:border-primary/50 transition-colors">
                            <div className="space-y-0.5">
                                ...
                                <p className="text-[11px] text-muted-foreground">Activar o desactivar marca</p>
                            </div>
                            <label className="relative inline-flex items-center cursor-pointer">
                                <input 
                                    type="checkbox" 
                                    className="sr-only peer"
                                    checked={currentMarca.activo !== false}
                                    onChange={(e) => setCurrentMarca({ ...currentMarca, activo: e.target.checked })}
                                />
                    ...
                    <div className="flex justify-end space-x-3 pt-6 mt-6 border-t border-border/50">
                        ...
                        >
                            <Save className="w-4 h-4" /> {currentMarca.id ? 'Guardar Cambios' : 'Crear Marca'}
                        </button>
                    </div>
                </form>
            </Modal>
            ...
            >
                <div className="space-y-6">
                    <div className="flex items-start gap-4 p-4 bg-destructive/10 text-destructive rounded-lg border border-destructive/20">
                        ...
                        <div>
                            <h4 className="font-semibold text-sm">Esta marca pasará a estado Inactivo</h4>
                            <p className="text-xs mt-1 opacity-90 text-destructive/80">Esta marca dejará de estar disponible para el registro de nuevos productos, pero no se borrará para preservar el historial.</p>
                        </div>
                    </div>
                                ...
                                <tr key={cat.id} className="hover:bg-accent/30 transition-colors group">
                                    ...
                                    <td className="p-4 text-sm">
                                        <div className="flex justify-end gap-2 transition-opacity">
                                            <button
                                                onClick={() => { setCurrentMarca(cat); setIsEditing(true); }}
                                                title="Editar"
                                            ...
                                            {cat.activo ? (
                                                ...
                                                <button
                                                    onClick={() => updateMutation.mutate({ ...cat, activo: true } as Marca, {
                                                        onSuccess: () => toast.success('Marca activada con éxito')
                                                    })}
                                                    title="Activar"
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-between p-4 border-t bg-muted/20">
                        <span className="text-sm text-muted-foreground">
                            Mostrando {((currentPage - 1) * itemsPerPage) + 1} a {Math.min(currentPage * itemsPerPage, filteredMarcas.length)} de {filteredMarcas.length}
                        </span>
                        <div className="flex items-center gap-2">
                            <button 
